...
import agrolens.utils
from agrolens.models.abstract import ModuleTemplate
...
class U2Net(ModuleTemplate):
    ...
    The :class:`U2Net <agrolens.models.sod.U2Net>` class implements basic methods
    ...
    def train(self, train_data_fpath, batch_size=8, epoch=10000, cpu=8, gpu=1,
    ...
    def inference(self, image_path, strategy='resize', batch_size=8, cpu=4, gpu=1,
        ...
        if cpu == 1:
            ...
            imganns = joblib.Parallel(n_jobs=cpu, backend='multiprocessing')(
                joblib.delayed(self._inference_post_process)(
                    images_fpath[i], pred_masks[i], u_cutoff, image_opening_kernel, image_closing_kernel
                ) for i in tqdm.tqdm(range(len(pred_masks)), desc='Post-processed images: '))
        
        imganns = agrolens.utils.ImageAnnotations(imganns)
    ...
    def _inference_post_process(self, image_fpath, pred_mask, u_cutoff, image_opening_kernel, image_closing_kernel):
        ...
        return agrolens.utils.ImageAnnotation(image_fpath, pred_mask, 'array')
